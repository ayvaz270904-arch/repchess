...
- **«Правка не работает» → сначала проверить, дошёл ли код.** `sw.js` отдаёт HTML network-first, но при неудачном запросе **молча** возвращает копию из кэша — а тренерка открывается через VPN (`*.workers.dev` режут РФ-провайдеры), так что старая страница выглядит как рабочая. Владелец смотрит `APP_VERSION` в «Админ → 📦 Версия» и жмёт «Проверить обновление» (`checkAppUpdate` — тянет файл мимо кэша, чистит `caches` и обновляет SW). **`APP_VERSION` в trainer.html обновлять вместе с заметными правками**, иначе проверка бесполезна. Со своей стороны сверять так: `curl -s <url>/trainer.html | grep APP_VERSION` и `shasum` против локального файла — 09.09.2026 это сразу показало, что деплой не при чём.
...
- **Booking with a friend (09.04-guest).** One guest per person. Stored on the groupLessons doc as two parallel string arrays — `guestOf[i]` = email of the host, `guestNames[i]` = guest's name (may be ''). A guest **takes a seat**: `joinGroup` checks `emails.length + guestOf.length + add <= max`, needs `add` lessons in the matching category AND in the total remainder, and the cabinet payload sends `count` = students + guests (без этого группа на 8 тихо становится на 10). Already-joined + guest = adds only the guest (no "already" error); `leaveGroup` removes the host's guests too. Firestore `count` field stays students-only — trainer.html writes it as `grpSelectedStudents.length`, so both sides don't fight over one field. The guest is **charged at attendance**, not at booking: `_prefillGuests(g)` in trainer.html fills the «Гости — списать с ученика» rows from `guestOf`/`guestNames` (saved `guestCharges` win if present), and submitting attendance creates an `isGuestCharge` lesson with status `done` on the host's email. **Deploy order: bot first, then client** — an old bot silently ignores `guest=1` and books one person while the new cabinet says «записаны вдвоём».
- **«Номер покупок» ≠ телеграмный номер (09.10-phone).** Баланс ищется ПО ТЕЛЕФОНУ (`clientBalance` → `usePhone`), почта — только запасной ключ; поэтому у купившего на сайте с другого номера кабинет пуст, и «✉️ Изменить почту клиента» это НЕ лечит. Клиент шлёт заявку из профиля (`action=claimPhone`), бот проверяет наличие оплат на номере + что он не занят другим кабинетом (≤3 попыток в сутки), админ подтверждает кнопкой `ph:ok`. После подтверждения `link.phone` = номер из оплат, а телеграмный уезжает в **`link.tgPhone`**. Инварианты: `saveLink` сохраняет `tgPhone` сам (не терять при любых вызовах); повторное «Поделиться номером» при наличии `tgPhone` НЕ возвращает телеграмный номер; метка подарка акции `pg20_<цифры>` переносится на новый номер. Почту меняем только гостю — у привязанного на ней висят занятия в Firestore.
- **Вход по номеру: только кнопкой (09.11-phonebtn).** `request_contact` — единственный способ получить ПОДТВЕРЖДЁННЫЙ номер; текстом принимать нельзя (чужой номер открыл бы чужие покупки). Клавиатура больше не `one_time_keyboard` — она пряталась, и люди слали номер текстом, а тот уходил в общий авто-ответ «Спасибо, я передал команде». Теперь `_looksLikePhone` (11 цифр с 7/8 или 10 с 9, коротко, мало букв) ловит такое сообщение: непривязанному — объяснение + клавиатура обратно, привязанному — путь в «Профиль → Номер для поиска покупок».
